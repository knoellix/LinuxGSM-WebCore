...
# Process priority helpers — install/download is heavy disk + network, so we
# always run as PRIO_LOW (idle ionice, nice=10). Game-server starts later
# get PRIO_HIGH from steamcmd_control.sh.
...
# jobs.pl _kill_job_processes() can be triggered by job-list iteration when
# status moves out of "running". Always remove pgid before writing the final
# status; on_exit covers SIGTERM / unexpected aborts. Same lesson as
# steamcmd_control.sh — see CLAUDE.md §8.12.
...
echo "=== steamcmd_install revision: 2026-05-02-prio-v8 ==="
...
if [ "${RUNTIME_MODE:-}" = "wine" ]; then
    FORCE_PLATFORM_ARGS="+@sSteamCmdForcePlatformType windows +@sSteamCmdForcePlatformBitness 64"
fi
...
mkdir -p "$INSTALL_DIR"
chown "$UNIX_USER":"$UNIX_USER" "$INSTALL_DIR" 2>/dev/null || true
chown -R "$UNIX_USER":"$UNIX_USER" "$SERVER_DIR" 2>/dev/null || true

VALIDATE_FLAG=""
[ "$VALIDATE" = "validate" ] && VALIDATE_FLAG="validate"

STEAMCMD="${STEAMCMD_PATH:-steamcmd}"
echo "Using SteamCMD: $STEAMCMD (priority prefix: $PRIO_LOW)"

# PRIO_LOW for the actual download — keeps disk + CPU available for any
# running game server on the same host. nice/ionice values inherit through
# su(1) into steamcmd's child processes.
if ! $PRIO_LOW su -s /bin/bash -c "
    '$STEAMCMD' $FORCE_PLATFORM_ARGS +force_install_dir '$INSTALL_DIR' \
                +login anonymous \
...
cleanup_stale_wine_runtime() {
    ...
    # Only touch clearly stale processes (older than 5 minutes) to avoid
    # terminating the active setup run.
...
EOF
        ...
        chown "$UNIX_USER":"$UNIX_USER" "$WINE_SETUP_SCRIPT" 2>/dev/null || true
        # Wine runtime setup is heavy IO (vc_redist, wineboot caches) — also PRIO_LOW.
        if ! $PRIO_LOW su -s /bin/bash -c "bash '$WINE_SETUP_SCRIPT'" "$UNIX_USER"; then
            echo "hint_wine_required" > "$JOB_DIR/error_hint"
            set_final_status "failed"
            exit 1
        fi
        touch "$WINE_READY_MARKER"
        chown "$UNIX_USER":"$UNIX_USER" "$WINE_READY_MARKER" 2>/dev/null || true
...
case "\${1:-}" in
    ...
    restart|update|backup|console|debug)
        # Refuse LGSM-style management actions: this wrapper has no logic to
        # talk to steamcmd or coordinate background workers. The real path is
        # via Webmin (steamcmd_control.sh / steamcmd_install.sh).
...
if [[ "\$BIN_NAME" == *.exe ]]; then
    ...
    export WINEARCH=win64
    # Keep launch environment close to the proven manual start path.
    # Overly strict DLL overrides can break game-specific runtime behavior.
    unset WINEDLLOVERRIDES
    unset WINEDEBUG
    # Windrose must use one fixed launch path, identical to manual start.
...
chown "$UNIX_USER":"$UNIX_USER" "$LAUNCH_CMD_FILE" "$LAUNCH_WRAPPER" 2>/dev/null || true
if [ -n "$SCRIPT_NAME" ]; then
    chown "$UNIX_USER":"$UNIX_USER" "$SERVER_DIR/$SCRIPT_NAME" 2>/dev/null || true
fi
